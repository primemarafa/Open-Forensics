...
import { Clock, CheckCircle2, Target, ArrowRight, HardDrive, Cpu, FileText, Network, Globe, Database } from 'lucide-react';
...
const getCategoryIcon = (category?: string) => {
  switch (category) {
    case 'disk': return <HardDrive className="w-4 h-4 text-cyan-400" />;
    case 'memory': return <Cpu className="w-4 h-4 text-blue-400" />;
    case 'log': return <FileText className="w-4 h-4 text-emerald-400" />;
    case 'timeline': return <Clock className="w-4 h-4 text-rose-400" />;
    case 'network': return <Network className="w-4 h-4 text-purple-400" />;
    case 'browser': return <Globe className="w-4 h-4 text-amber-400" />;
    case 'registry': return <Database className="w-4 h-4 text-indigo-400" />;
...
export default function ScenarioCard({
  ...
  return (
    <div
      ...
      className="group relative flex flex-col h-full bg-slate-900/60 hover:bg-slate-900/80 backdrop-blur-xl border border-slate-800/80 hover:border-cyan-500/40 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_10px_30px_-10px_rgba(6,182,212,0.15)] hover:-translate-y-1 cursor-pointer overflow-hidden"
    >
      {/* Top subtle ambient highlight */}
      <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-500/0 to-transparent group-hover:via-cyan-500/70 transition-all duration-500"></div>
      ...
      <div className="flex items-center justify-between gap-2 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {isCompleted && (
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-xs font-mono font-bold">
      ...
      {/* Title */}
      <h3 className="text-lg font-bold text-slate-100 group-hover:text-cyan-400 transition-colors mb-2.5 leading-snug">
