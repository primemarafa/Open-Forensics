            ...
            {currentQ.options?.map((opt, i) => (
              <label
                ...
                className={`flex items-center p-3.5 rounded-xl border cursor-pointer transition-all ${
                  answers[currentQ.id] === opt.value
                    ? 'border-cyan-500 bg-cyan-500/10 text-white font-medium shadow-[0_0_10px_rgba(6,182,212,0.15)]'
              ...
              >
                <input
                  ...
                  className="w-4 h-4 text-cyan-500 border-slate-700 bg-slate-800 focus:ring-cyan-500"
        ...
        return (
          <div className="mt-4">
            <input
              ...
              onChange={(e) => handleAnswerChange(e.target.value)}
              disabled={showResults}
              className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-sm text-slate-100 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition-all disabled:opacity-50 font-mono"
        ...
        return (
          <div className="mt-4">
            <input
              ...
              onChange={(e) => handleAnswerChange(e.target.value)}
              disabled={showResults}
              className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-sm text-slate-100 focus:border-cyan-500 outline-none disabled:opacity-50 font-mono"
            />
          </div>
        );
    }
    ...
    <div className="bg-slate-900/90 rounded-xl border border-slate-800 p-6 flex flex-col h-full font-sans">
      ...
      <div className="flex justify-between items-center mb-5 border-b border-slate-800 pb-3">
        <h2 className="text-base font-bold text-slate-100 font-mono flex items-center gap-2">
          ...
          <span>{t('common.question', { defaultValue: 'Question Technique' })} {currentIdx + 1}</span>
        ...
        {showResults ? (
          <div className={`flex items-center space-x-1.5 px-3 py-1 rounded-full text-xs font-mono font-bold ${
            results[currentQ.id] ? 'bg-cyan-500/20 text-cyan-400' : 'bg-rose-500/20 text-rose-400'
          }`}>
            {results[currentQ.id] ? (
          ...
          <span className="text-xs font-mono text-slate-400">
            Points : <strong className="text-cyan-400">{currentQ.points || 25} pts</strong>
          </span>
        )}
      </div>
      ...
      <div className="flex-grow overflow-y-auto pr-1 space-y-4">
        ...
        {!showResults && (
          <div className="pt-2">
            ...
            >
              ...
              <span>{revealedHints[currentQ.id] ? 'Masquer l\'indice' : 'Besoin d\'un indice forensique ?'}</span>
            </button>
            ...
            {revealedHints[currentQ.id] && (
              <div className="mt-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-xs text-amber-200 leading-relaxed font-sans animate-in fade-in">
                💡 <strong>Conseil d'investigateur :</strong> Utilisez les commandes Volatility (ex: <code>vol.py windows.malfind</code> ou <code>windows.pslist</code>) dans le terminal pour confronter les adresses mémoires et les PIDs parents/enfants.
              </div>
            )}
          </div>
        )}
        ...
        {showResults && (
          <div className={`mt-5 p-4 rounded-xl border ${
            results[currentQ.id] ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-rose-500/10 border-rose-500/30'
          }`}>
            <h4 className="font-bold text-xs uppercase tracking-wider font-mono text-slate-200 mb-1.5 flex items-center gap-1.5">
              ...
              {t('common.explanation', { defaultValue: 'Explication Forensique' })} :
            </h4>
      ...
      <div className="flex justify-between items-center mt-6 pt-4 border-t border-slate-800">
        ...
        {!showResults && currentIdx === questions.length - 1 ? (
          <button
            ...
            className="flex items-center px-5 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl transition-all font-mono font-bold text-xs shadow-[0_0_15px_rgba(6,182,212,0.3)] cursor-pointer"
          >
            <Send className="w-3.5 h-3.5 mr-2" />
            {t('common.submit', { defaultValue: 'Clôturer le Cas DFIR' })}
          </button>
        ) : (
          <button
            onClick={handleNext}
